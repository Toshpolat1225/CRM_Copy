window.addEventListener("load", function(e) {
    ...
    function Currency() {
        ...
        document.querySelectorAll(".dailyPayment").forEach((elem) => {
            elem.textContent = toCurrency(elem.textContent);
        });
    }
    AddWorkers();
                ...
                fetch("/projects/add/worker", {
                    ...
                    .then((workers) => {
                        const html = workers.map((c) => {
                            return ` <option value="${c._id}">${c.fullname}</option> `;
                        });
                        worker.innerHTML = ` <div class="form-group col-xl-6 col-10 col-sm-11">
